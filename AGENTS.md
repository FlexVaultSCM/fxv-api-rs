...
- `message.version` is the payload schema's `major.minor` version **on that `kind`'s own timeline**, not a global one. Kinds move independently and do not all sit at the same version; read the current one off the payload's `VersionedJsonMessage::version()` in fxv-core rather than assuming. It is a string so `1.10` can't be read as `1.1`; `MessageVersion` parses it. On the CLI side each payload's `VersionedJsonMessage::version()` supplies it, and fxv-core's `cargo insta` snapshots fail if a payload's shape moves without the version moving.
